...
# ###############################################################################################################################################
# Você deve conter essas funções criadas em arquivos separados para funcionar e que estejam no mesmo diretório inclusive esse arquivo principal:

@include "criarSubscriber.awk"
@include "enviarFluxos.awk"
@include "funcaoNotificarWhatsApp.awk"
@include "deletarSubscriber.awk"
#################################################################################################################################################
...
BEGIN {
    ...
    home = ENVIRON["HOME"]
    converterArquivoXlsx = "soffice --convert-to xlsx /home/Área de Trabalho/leadsElane/"nomeArquivo""dataAtual".csv"
    arquivo = "/home/brenner/\"Área de Trabalho\"/leadsElane/"nomeArquivo""dataAtual".csv"
...

{
    ...
    registro += 1
    if (NR == 1){
        print "nome","telefone","etiquetas","campanha" > home "/Área de Trabalho/leadsElane/" nomeArquivo dataAtual".csv"
    }
    ...
    if (length($2) < 11){
        ...
        dd = substr($2, 1, 2)
        numero = substr($2, 3)
        print (tolower($1),"55"dd"9"numero,"leadsElane"elaneNumero,toupper(campanha)) > home "/Área de Trabalho/leadsElane/" nomeArquivo dataAtual".csv"
        ...
        notificarWhatsApp("DIB","385910829",textoNotificacao)
        system("sleep 5s")


    }
    else{
        print (tolower($1),"55"$2,"leadsElane"elaneNumero,toupper(campanha)) > home "/Área de Trabalho/leadsElane/" nomeArquivo dataAtual".csv"
...
END {
    ...
    textoNotificacao = "*TOTAL DE LEADS:("dataAtual")*" "\\n\\n"registro""
    notificarWhatsApp("DIB","385910829",textoNotificacao)
